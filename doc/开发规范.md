...
| 技术 | 版本 | 说明 |
|------|------|------|
| **RuoYi-Cloud** | 基础框架 | Spring Boot 3.3.5 |
| **Spring Boot** | 3.3.5 | 核心框架，提供自动配置和快速开发能力 |
| **Spring Cloud** | 2023.0.3 | 微服务框架，提供服务治理能力 |
| **Java** | 17+ | JDK版本，建议使用 JDK 17 或更高版本 |

### 1.2 微服务组件

| 组件 | 版本 | 说明 |
|------|------|------|
| **Spring Cloud Gateway** | 4.1.5 | API网关，负责双路由转发、负载均衡、限流、鉴权 |
| **Spring Cloud OpenFeign** | 4.1.3 | 服务间调用，基于HTTP的声明式调用 |
| **Spring Cloud Alibaba Nacos** | 2023.0.1.2 | 注册中心与配置中心 |
| **Spring Cloud Alibaba Sentinel** | 2023.0.1.2 | 限流熔断（可选） |
| **Micrometer Tracing** | 1.3.2 | 链路追踪（可选集成） |
...
|------|------|------|
| **MyBatis** | 3.0.3 | ORM框架 |
| **Druid** | 1.2.27 | 数据库连接池（支持SQL监控） |
| **Hutool** | 5.8.34 | Java工具类库 |
| **Lombok** | 1.18.36 | 代码简化，减少样板代码 |
| **Fastjson2** | 2.0.54 | JSON处理库 |

#### 1.4.1 数据库连接池选择：Druid vs HikariCP
...
**选择建议**：
- **本项目选择Druid**：因为Spring Boot默认集成，性能优秀，满足微服务架构需求
...
├── code/                         # 代码与 Maven 工程目录
│   ├── pom.xml                   # 父POM (groupId: com.sia, artifactId: qa-hero)
│   ├── qa-hero-gateway/          # API网关（双路由）
│   ├── qa-hero-auth/             # 管理端认证服务
│   ├── qa-hero-modules/          # 业务模块目录
│   │   ├── qa-hero-system/       # 系统服务（管理端）
│   │   ├── qa-hero-auth-app/     # 业务端认证服务 ⭐ 新增
...
### 2.4 业务服务通用结构（以 qa-hero-auth-app 为例）
...
└── src/
    └── main/
        ...
        │           └── auth/
        │               └── app/
        │                   ├── QaHeroAuthAppApplication.java  # 启动类
...
**注意**：
- 业务端服务与管理端服务隔离，使用独立的实体类
- 配置从 Nacos 读取，`bootstrap.yml` 只配置 Nacos 地址和服务名

### 2.5 包命名规范

- **根包名**：`com.sia`
- **管理端模块包名**：`com.sia.{module}`，如 `com.sia.system`、`com.sia.auth`
- **业务端模块包名**：`com.sia.{module}.app`，如 `com.sia.auth.app`、`com.sia.content.app`
- **公共模块包名**：`com.sia.common.{module}`，如 `com.sia.common.core`
- **API模块包名**：`com.sia.api.{module}`，如 `com.sia.api.system`
