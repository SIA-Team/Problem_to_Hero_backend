...
本项目采用Maven多模块结构，便于模块化管理与依赖管理。**所有代码与 Maven 模块均位于 `code/` 目录下**，SQL 脚本位于仓库根目录的 `sql/`。
...
└── src/
    └── main/
        ...
        │                   ├── enums/                      # 枚举类（包名 enums，避免与关键字 enum 冲突）
...
└── src/
    └── main/
        ...
        └── resources/
            ...
            └── application-dev_template.yml              # 开发环境配置模板（敏感信息不提交）
```
...
│               │   # 注意：MyBatis配置已在common-dbservice中统一配置
        ...
        └── resources/
            ...
            ├── application-dev.yml                       # 开发环境配置（可提交或由模板复制）
            ├── application-dev_template.yml              # 开发环境配置模板（敏感信息占位，不提交密码等）
